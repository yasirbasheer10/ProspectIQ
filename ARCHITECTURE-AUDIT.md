...
**Repo hygiene.** Eleven scratch files sit in the project root: `test-groq.js`,
`test-groq-list.js`, `test-groq-model.js`, `scratch_serper.ts`, `test.ts`, `test-db.ts`,
`test-dashboard.ts`, `test-intelligence.ts`, `proxy.ts`, `prospectiq-fixes-1-4.patch` and
`supabase_migration.sql`. Also `AGENTS.md` contains a `nextjs-agent-rules` block asserting that
...
12. Delete `lib/ai/providers.ts`, `lib/ai/provider.ts`, `ScoreRing.tsx`, `Skeleton.tsx`, the three
    unused dependencies and the eleven root scratch files. Rename `gemini.ts` to `groq.ts` and
    centralise the model constant.
